...
k () {
  ...
  typeset -a o_all o_almost_all o_human o_si o_directory o_group_directories \
	  o_no_directory o_no_vcs o_sort o_sort_reverse o_help o_absolute_colors
  zparseopts -E -D \
             a=o_all -all=o_all \
  ...
  do
    ...
    do
      ...
      # --------------------------------------------------------------------------
      (( UID == nsv[uid] )) ||
        OWNER=$'\e[38;5;'"${K_CUSTOM_OWNER_COLOR:-241}m$OWNER"$'\e[0m'
      (( usergroups[(r)${nsv[gid]}] )) ||
        GROUP=$'\e[38;5;'"${K_CUSTOM_GROUP_COLOR:-241}m$GROUP"$'\e[0m'
